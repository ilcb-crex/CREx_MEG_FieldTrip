        ...
        if isfield(struct_data.svg,'Filter')
            ...
            for i = 1:length(filter)
                ...
                try
                    ...
                    if ~isempty(strfind(lower(filter(i).Subfilter.Result), lower(predefinedSources)))
                    ...
                    error([' Error is caused by filter type ''', filter(i).Subfilter.Type '''.']);
                end
            end
            fprintf(fid,'  </filter>\n');
            fprintf(fid,'</defs>\n');
...
if isfield(svgstruct, svgfield)
    if ischar(svgstruct.(svgfield))
        % The strmatch below is a very inefficient search (Matlab limitation)
        if sum(strcmp(svgstruct.(svgfield), resultStrings))==0
...
if isfield(svgstruct, svgfield)
    if ischar(svgstruct.(svgfield))
        if sum(strcmp(svgstruct.(svgfield), list))==0
...
if strcmp(get(ax,'Visible'),'on')
   ...
   % linewidth = get(ax,'LineWidth');
    ...
    if length(axxtick) > 1
        ...
        for stick = [2*axxtick(1)-axxtick(2) axxtick]
            minor_axxtick = [minor_axxtick minor_lin_sticks + stick]; %#ok
        end
    ...
    if length(axytick) > 1
        ...
        for stick = [2*axytick(1)-axytick(2) axytick]
            minor_axytick = [minor_axytick minor_lin_sticks + stick]; %#ok
        end
    ...
    if length(axztick) > 1
        ...
        for stick = [2*axztick(1)-axztick(2) axztick]
            minor_axztick = [minor_axztick minor_lin_sticks + stick]; %#ok
        end
   ...
   %%% [axxtick, minor_axxtick] = tick_islog('x');
    ...
    if strcmp(get(ax,'XScale'),'log')
        ...
        if ~isempty(axxtick)
            ...
            for stick = all_axxtick
                minor_axxtick = [minor_axxtick minor_log_sticks + stick]; %#ok
            end
        end
        minor_axxtick = minor_axxtick(minor_axxtick > min(axlimx) & minor_axxtick < max(axlimx));
    end
    if strcmp(get(ax,'YScale'),'log')
        ...
        if ~isempty(axytick)
            ...
            for stick = all_axytick
                minor_axytick = [minor_axytick minor_log_sticks + stick]; %#ok
            end
        end
        minor_axytick = minor_axytick(minor_axytick > min(axlimy) & minor_axytick < max(axlimy));
    end
    if strcmp(get(ax,'ZScale'),'log')
        ...
        if ~isempty(axztick)
            ...
            for stick = all_axztick
                minor_axztick = [minor_axztick minor_log_sticks + stick]; %#ok
            end
        end
        minor_axztick = minor_axztick(minor_axztick > min(axlimz) & minor_axztick < max(axlimz));
    end
...
for i=length(axchild):-1:1
    ...
    elseif strcmp(get(axchild(i),'Type'),'rectangle')
        ...
        curv = get(axchild(i),'Curvature');
        curvature = zeros(2,1);
        curvature(1) = curv(1)*rect(3)*0.5;
        if length(curv) > 1
            curvature(2) = curv(2)*rect(4)*0.5;
        else
    ...
    elseif strcmp(get(axchild(i),'Type'),'image')
        ...
        if (size(pointx, 1) > 1) && (size(pointy, 1) > 1)
            ...
            [~,y,z] = project(zeros(size(pointy)),pointy,zeros(size(pointy)),projection);
        end
        pointc=get(axchild(i),'CData');
...
function patch2svg(fid,group,axpos,xtot,ytot,scolorname,style,width, edgecolorname, face_opacity, edge_opacity, closed) %#ok
if closed
    type = 'polygon';
else
    type = 'polyline';
...
% 
% function [axtick, minor_axtick] = tick_islog(ax)
%     
%     if strcmp(get(ax,[upper(ax),'Scale']),'log')
%         axtick = log10(get(ax,[upper(ax),'Tick']));
%         minor_axtick = [];
%         if ~isempty(axxtick)
%             all_axxtick = axxtick(1):1:axxtick(end); 
%             for stick = all_axxtick
%                 minor_axtick = [minor_axtick minor_log_sticks + stick]; %#ok
%             end
%         end
%         minor_axtick = minor_axtick(minor_axtick > min(axlimx) & minor_axtick < max(axlimx));
%     end
