...
if options.aa_factor == 0
    %--- CREx bug correction : Error using hg.figure/get The name 'GraphicsSmoothing' 
    % is not an accessible property for an instance of class 'figure'.
    sfig = get(ancestor(fig, 'figure'));
    isgs = 0;
    if isfield(sfig, 'GraphicsSmoothing')
        if strcmp(sfig.GraphicsSmoothing, 'on')
            isgs = 1;
        end
    end
    options.aa_factor = 1 + 2 * (~(isgs) | (options.renderer == 3));
    %--- CREx remove ~(using_hg2) --> graphicsversion will be removed in
