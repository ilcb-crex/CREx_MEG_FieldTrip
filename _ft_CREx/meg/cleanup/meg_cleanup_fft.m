...
% Trois types de figures generees pour controler les donnees au niveau de
% chaque canal :
...
if ok
    ...
    fdossp = make_dir([dirpath,filesep,'FFT_plots_',datatyp],1);    
...
% %     save([fdossp,filesep,'FFT_',datatyp,'Data',suff],'freq','allFFT','label')
    ...
    if ~isempty(allFFTstack)
        meg_fftstack_fig(ftData, freqst, allFFTstack, spsparam, datapath, fdossp, datatyp) 
        namstk=['FFTstack_',datatyp,'Data',suff,'_N',num2str(spsparam.n),'DUR',num2str(round(spsparam.dur)),'s'];
        save([fdossp,filesep,namstk],'freqst','allFFTstack','spsparam','label')
    end
    
    if strcmpi(datatyp,'filt')==1
        % Figure avec des zooms dans les donnees pour mieux voir si probleme
        fdosz = make_dir([dirpath,filesep,'DataDisp_',datatyp],1);    
