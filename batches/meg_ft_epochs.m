...
dftopt = [];
dftopt.datatyp = 'clean';
dftopt.trigfun = 'define_pure_triggevent'; % Function to define triggers codes    
% define_bapaseeg_triggevent
dftopt.trialfun = [];       % Special function to sort events depending on responses (ex. : 'ADys_trialfun';)

dftopt.prestim = 0.200;     % Pre-stimulus time (s): duration before stimulus onset - positive number
dftopt.postim = 0.650;      % Post-stimulus time (s) : duration after stimulus onset

dftopt.bsl = [-0.200 0];    % Baseline use to demean (ex. ADys : [-0.5 -0.3])
dftopt.dofig = 1;           % Flag to generate figures of epochs

%_ doAvg : trials to average 
avgopt = [];
avgopt.datatyp = 'allTrials_ICAcomp';

%_doExtract & _doAvgT : post-processing options to apply to trials 
% For doExtract : processing done on epochs after their extraction from
% continuous data
% For doAvg : processing done on the average across trials 
trialopt = [];
...
rmtopt = [];
rmtopt.input = 'manual'; %'mat'; %'manual'; 
...
% ERF ICA component figures
icafig = [];
icafig.datatyp = 'avgComp_'; % + ajouter preproc to find
...
% 
if doAvg==1
    trialopt.datatyp = avgopt.datatyp;
    for np = 1 : Ndp
        disp_progress(np, Ndp);
...
if doICAonT==1
    for np = 1 : Ndp
        disp_progress(np, Ndp);
        meg_ica_proc(datapaths{np}, icaopt);
    end
end

if doSupAvgICA==1
   % Need an avgComp data structure with data in fields indicating the 
   % conditions
    for np = 1 : Ndp
        disp_progress(np, Ndp);
        fprintf('\nProcessing of data in :\n%s\n\n', datapaths{np});
